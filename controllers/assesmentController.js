...
export const createAssesment = async (req, res) => {
    ...
    try {
        const { assesmentName, totalMark } = req.body;
        ...
        const result = await conn.query('INSERT INTO assesments (assesmentName, totalMark, courseId) VALUES (?, ?, ?)', [assesmentName, totalMark, courseId]);
        const assesmentId = result[0].insertId;
        res.status(201).json({
